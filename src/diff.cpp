...
std::vector<std::string> LCS_HIRSHBERG(const std::vector<std::string>& words_x, const std::vector<std::string>& words_y) {
    size_t len_x = words_x.size();
    if (len_x == 0) {
        std::vector<std::string> empty;
        return empty;
    } else if (len_x == 1) {
        if (std::find(words_y.begin(), words_y.end(), words_x[0]) != words_y.end()) {
        ...
        std::vector<int> temp = lcs_length(std::vector<std::string>(x_end.rbegin(), x_end.rend()), std::vector<std::string>(words_y.rbegin(), words_y.rend()));
        std::vector<int> l2(temp.rbegin(), temp.rend());

        std::vector<int> sum_l(l1.size());
        std::transform(l1.begin(), l1.end(), l2.begin(), sum_l.begin(), std::plus<int>());
